...
const P = {
  ...
  blueDarker: 'oklch(0.34 0.18 252)',
...
const Logo = () => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
    <div style={{
      ...
      background: `linear-gradient(140deg, ${P.blue}, ${P.blueDeep})`,
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      boxShadow: `0 18px 38px -10px ${P.blueDeep}66, 0 2px 4px ${P.blueDeep}33`,
      fontFamily: `'Inter', sans-serif`, fontWeight: 900, fontSize: 26, color: '#fff', letterSpacing: -1.2,
    }}>BR</div>
...
const PassEye = ({ show, onToggle }) => (
  <button type="button" onClick={onToggle} style={{ background: 'none', border: 0, cursor: 'pointer', padding: '0 13px 0 4px', display: 'flex', alignItems: 'center', color: P.ink3 }}>
    {show ? (
      ...
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
        <path d="M17.94 17.94A10 10 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9 9 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24M1 1l22 22"/>
      </svg>
    )}
  </button>
);
...
const InputField = ({ label, type = 'text', value, onChange, placeholder, icon, rightSlot, onKeyDown, autoComplete, maxLength }) => {
  ...
  const borderColor = focus ? P.blue : P.line2;
  return (
    <div>
      {label && <div style={{ fontSize: 12, fontWeight: 700, color: P.ink2, marginBottom: 6 }}>{label}</div>}
      <div style={{ position: 'relative', background: P.surfaceAlt, border: `1.5px solid ${borderColor}`, borderRadius: 16, transition: 'border-color 0.15s', boxShadow: focus ? `0 0 0 4px ${P.blue}12` : 'none', display: 'flex', alignItems: 'center' }}>
...
const PrimaryBtn = ({ onClick, children, loading, disabled }) => (
  <button onClick={onClick} disabled={loading || disabled} style={{
    ...
    border: 0, borderRadius: 16, cursor: (loading || disabled) ? 'not-allowed' : 'pointer',
    color: '#fff', fontFamily: thFont, fontSize: 15, fontWeight: 700,
    boxShadow: `0 8px 22px -6px ${P.blueDeep}80`,
...
const SuccessBox = ({ msg }) => msg ? (
  <div style={{ padding: '10px 13px', borderRadius: 11, background: P.successSoft, border: `1px solid ${P.success}30`, color: P.success, fontSize: 12.5, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="m4 12 5 5L20 6"/></svg>
    {msg}
  </div>
) : null;
...
const LoginScreen = ({ onLogin }) => {
  ...
  const [forgotOtp, setForgotOtp] = useState(''); // the generated OTP (stored in memory)
    ...
    if (code && state === 'br_cargo_line') {
      ...
      supabase.functions.invoke('lineLogin', { body: { code, redirect_uri: LINE_REDIRECT_URI } })
        ...
        .then(res => {
          ...
          if (data.success) {
            const u = {
              email: data.user.email || `line_${data.user.line_user_id}@line.user`,
  ...
  const handleEmailLogin = async () => {
    ...
    setLoading(true);
    try {
      // ใช้ base44.auth.login() จริง — ตรวจสอบรหัสผ่านกับ backend
      await base44.auth.login({ email: e, password: pw });
      const user = await base44.auth.me();
      // จดจำอีเมลถ้าติ๊ก "จดจำฉัน"
      if (rememberMe) {
        try { localStorage.setItem('br_remember_email', e); } catch {}
      } else {
        try { localStorage.removeItem('br_remember_email'); } catch {}
      }
      try { localStorage.setItem('br_session_user', JSON.stringify(user)); } catch {}
  ...
  const handleRegister = async () => {
    ...
    try {
      const users = await base44.entities.User.filter({ email: e.trim().toLowerCase() });
  ...
  // Step 3: ตั้งรหัสผ่านใหม่จริงผ่าน base44.auth.updateMe
  const handleResetPassword = async () => {
    ...
    setLoading(true);
    try {
      await base44.auth.updateMe({ password: newPw });
      setStep('forgot-success');
    } catch {
      setResetErr('เกิดข้อผิดพลาด กรุณาลองใหม่');
    }
    setLoading(false);
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', userSelect: 'none' }}>
          <div onClick={() => setRememberMe(v => !v)} style={{
            ...
            border: `2px solid ${rememberMe ? P.blue : P.line2}`,
            background: rememberMe ? P.blue : 'transparent',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {/* Google Login */}
          <button onClick={() => base44.auth.loginWithProvider('google', window.location.pathname)} style={{
            flex: 1, padding: '11px 14px',
            background: P.surface, border: `1.5px solid ${P.line2}`, borderRadius: 14,
            cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            fontFamily: thFont, fontSize: 13.5, fontWeight: 700, color: P.ink,
          ...
          {/* LINE Login */}
          <button onClick={() => { window.location.href = LINE_AUTH_URL; }} style={{
            flex: 1, padding: '11px 14px',
            background: '#06C755', border: 'none', borderRadius: 14,
            cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            fontFamily: thFont, fontSize: 13.5, fontWeight: 700, color: '#fff',
            boxShadow: '0 1px 4px rgba(6,199,85,0.3)',
          }}>
  ...
  // ── Forgot — Enter OTP ─────────────────────────────────
    ...
    body = (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          <div style={{ width: 60, height: 60, borderRadius: 18, background: 'oklch(0.93 0.04 245)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={P.blue} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="11" width="16" height="10" rx="2"/><circle cx="12" cy="7" r="3"/></svg>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: P.ink }}>กรอกรหัส OTP</div>
            <div style={{ fontSize: 13, color: P.ink3, marginTop: 4, lineHeight: 1.5 }}>ส่งรหัสไปที่<br/><span style={{ fontWeight: 700, color: P.ink2 }}>{forgotEmail}</span></div>
          </div>
        </div>
        <ErrBox msg={otpErr}/>
        ...
        <div>
          <input
            type="text" inputMode="numeric" value={otpInput} onChange={e => setOtpInput(e.target.value.replace(/\D/g, '').slice(0, 6))}
            placeholder="000000" maxLength={6}
            onKeyDown={e => e.key === 'Enter' && handleVerifyOTP()}
            style={{
              width: '100%', textAlign: 'center', letterSpacing: 14, fontSize: 28, fontWeight: 800,
              fontFamily: `'Inter', sans-serif`, color: P.ink, padding: '14px 0',
              border: `2px solid ${otpErr ? P.danger : P.line2}`, borderRadius: 16,
              outline: 'none', background: P.surfaceAlt, boxSizing: 'border-box',
            }}
          />
          <div style={{ textAlign: 'center', marginTop: 8, fontSize: 12.5 }}>
            {otpTimer > 0
        ...
        {otpTimer === 0 && (
          <button onClick={() => { setOtpInput(''); setOtpErr(''); setStep('forgot'); }} style={{ background: 'transparent', border: 0, cursor: 'pointer', color: P.blue, fontFamily: thFont, fontSize: 13, fontWeight: 700, textAlign: 'center' }}>
            ขอรหัส OTP ใหม่
          </button>
        )}
      </div>
  ...
  // ── Forgot — Set New Password ──────────────────────────
    ...
    body = (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          <div style={{ width: 60, height: 60, borderRadius: 18, background: P.successSoft, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={P.success} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg>
          </div>
          <div style={{ textAlign: 'center' }}>
  ...
  return (
    <div style={{ fontFamily: thFont, minHeight: '100%', position: 'relative', overflow: 'hidden', background: `linear-gradient(180deg, #ddeeff 0%, #c8e0f8 60%, #a8cff0 100%)` }}>
